...
const getOtpStatusController = asyncHandler(async (req, res) => {
  ...
  const otpData = await redisClient.get(`temp_session:${tempSessionId}`);

  const expiryTime = JSON.parse(otpData?.expires_at);
  const timeLeft =
    expiryTime == null
      ? null
      : Math.floor((expiryTime - new Date()) / 1000 / 60);
...
const otpVerificationController = asyncHandler(async (req, res) => {
  ...
  const otpFromDb = await redisClient.get(`otp:${tempSessionId}`);
...
//   const otpExpiryTime = new Date().getTime() + OTP_EXPIRY_TIME;

//   const otpData = await generateOTPQuery(
//     user?.id,
//     tempSessionId,
//     "otp",
//     hashedOTP,
...
//   const data = {
//     otp_attempts: otpData?.rows[0]?.otp_attempts,
//     otp_created_at: otpData?.rows[0]?.otp_created_at,
//     screen: "otp",
//   };
